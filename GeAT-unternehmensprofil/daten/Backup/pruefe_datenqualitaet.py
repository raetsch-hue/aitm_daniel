...
def pruefe_konsistenz(k: Kontext, regeln) -> list[Befund]:
    ...
    for s in [s for s in t.spalten if k.typen.get(s) == "text"
              and not any(x in s.lower() for x in STICH_NAME)]:
...
def pruefe_aktualitaet(k: Kontext) -> list[Befund]:
    ...
    verlaufsspalten = [s for s in datumsspalten
                       if not any(x in s.lower() for x in STICH_GEBURT + STICH_FRIST)]
    ...
    for s in datumsspalten:
        if any(x in s.lower() for x in STICH_FRIST):
            continue                      # Fristen dürfen in der Zukunft liegen
    ...
    # A4 — Monate ohne einen einzigen Eintrag mitten in der Reihe
    treffer, beispiele = 0, []
    for s in verlaufsspalten:
        alle = sorted(als_datum(w) for w in t.spalte(s) if als_datum(w))
        if len(alle) < 30:
            continue
        # Die ältesten fünf Prozent abschneiden: ein dünner Altbestand erzeugt sonst
        # Dutzende leerer Monate und verdeckt die Lücke, um die es geht.
        daten = alle[int(0.05 * len(alle)):]
        monate = Counter((d.year, d.month) for d in daten)
        lauf, cur = [], (daten[0].year, daten[0].month)
        ende = (daten[-1].year, daten[-1].month)
        while cur <= ende:
            lauf.append(cur)
            cur = (cur[0] + 1, 1) if cur[1] == 12 else (cur[0], cur[1] + 1)
        leere = [m for m in lauf[1:-1] if monate[m] == 0]
        if leere:
            treffer += len(leere)
            beispiele.append("Spalte `%s`: %d Monate ohne Eintrag mitten in der Reihe (%s)"
                             % (s, len(leere),
                                ", ".join("%02d/%d" % (m[1], m[0]) for m in leere[:6])))
        # zusätzlich: auffällig dünne Wochen innerhalb der dichten Reihe
        tage = Counter(daten)
        spanne = (daten[-1] - daten[0]).days
        if spanne > 200:
            luecken = []
            for a, b in zip(daten, daten[1:]):
                if (b - a).days >= 14:
                    luecken.append((a, b, (b - a).days))
            for a, b, n in luecken[: k.n_beispiele]:
                treffer += 1
                beispiele.append("Spalte `%s`: %d Tage ohne Eintrag zwischen %s und %s"
                                 % (s, n, a.isoformat(), b.isoformat()))
    out.append(Befund("A4", "Aktualität", "Zeitliche Lücken in der Reihe",
                      "Monate ohne Eintrag innerhalb des dicht belegten Zeitraums "
                      "(ältestes Zwanzigstel abgeschnitten) und Abstände von mindestens "
                      "14 Tagen zwischen zwei aufeinanderfolgenden Einträgen",
                      treffer, "Zeitreihe je Datumsspalte", beispiele[: 2 * k.n_beispiele], set()))
    return out
